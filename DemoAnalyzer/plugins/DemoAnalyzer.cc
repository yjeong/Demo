...
class DemoAnalyzer : public edm::EDAnalyzer {
	...
	private:
		...
		edm::EDGetTokenT<pat::MuonCollection> selectedPatMuons;
	...
	//	minTracks_(iConfig.getUntrackedParameter<edm::InputTag>("minTracks"))
{
	...
	selectedPatMuons = consumes<pat::MuonCollection>(iConfig.getParameter<edm::InputTag>("Pmu"));
	globalMuons = consumes<reco::TrackCollection>(iConfig.getParameter<edm::InputTag>("Gmu"));
	...
	if(SaveHisto)demo_tree = new TFile("PAT.root","recreate");
	...
	//------------------------------Track Isolation----------------------------
	htrIso_P = new TH1D("htrIso_P","Track_Isolation_{+}",20,0,10);
	htrIso_M = new TH1D("htrIso_M","Track_Isolation_{-}",20,0,5);
	...
	tree->Branch("PatMuon","TLorentzVector[nm]",&V_PatMuon);
...
DemoAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
	...
	for(auto tr=Tracks->begin();tr!=Tracks->end();tr++,ntr++)
	{
	...
	{
		for(auto tot_tr=Tracks->begin();tot_tr!=Tracks->end();tot_tr++,tot_ntr++)
		{
	...
	edm::Handle<pat::MuonCollection> Pmu;
	iEvent.getByToken(selectedPatMuons,Pmu);
	...
	for(auto pmu=Pmu->begin();pmu!=Pmu->end();pmu++,pnmu++)
	{
		tempLeps.push_back(*pmu);
		...
		V_PatMuon[pnmu] = pmu->momentum().x(), pmu->momentum().y(), pmu->momentum().z(), pmu->energy();
	...
	if(tempLeps.size()==2)
	{
		for(auto l1=tempLeps.begin();l1!=tempLeps.end()-1;l1++)
			for(auto l2=l1+1;l2!=tempLeps.end(); l2++)
			{
				hmllpm->Fill((l1->p4()+l2->p4()).M());
				//mllpm=(l1->p4()+l2->p4()).M();
			}
		mllpm=(tempLeps[0].p4()+tempLeps[1].p4()).M();
	}
	unsigned int tot_pnmu=0;
	if(PmuptP>10&&PmuptM>10)
	{
		for(auto tot_pmu=Pmu->begin();tot_pmu!=Pmu->end();tot_pmu++,tot_pnmu++)
		{
	...
	for(auto tr=Tracks->begin();tr!=Tracks->end();tr++)
	{
		...
		if(fabs(tr->pt())>10)
		{
			...
			if(tr->charge()==1)
			{
				for(auto tr2=Tracks->begin();tr2!=Tracks->end();tr2++)
				{
	...
	for(auto pmu=Pmu->begin();pmu!=Pmu->end();pmu++)
	{
		if(fabs(pmu->pt())>10)
		{
			vpt.push_back(pmu->pt());
	...
	edm::Handle <reco::TrackCollection> Gmu;
	iEvent.getByToken(globalMuons,Gmu);
	//--------------------------------------------isopt of global muon--------------------
	unsigned int ngmu=0;
	int GnmuP=-1,GnmuM=-1;
	double GmuptP=0,GmuptM=0;
	for(auto gmu=Gmu->begin();gmu!=Gmu->end();gmu++,ngmu++)
	{
	...
	{
		for(auto tot_gmu=Gmu->begin();tot_gmu!=Gmu->end();tot_gmu++,tot_gnmu++)
		{
	...
	/*	-----------MET section
		...
		iEvent.getByToken(patMETs,pMET);
		unsigned int nmet=0;
